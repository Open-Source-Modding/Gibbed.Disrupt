...
{
    // Watch Dogs 2 (WD2) big file format — version 11.
    // Split from the unified BigFileV5 (which previously bundled V11/WD2 + V13/WDL).
    // This class is V11-only. WD2 archives are version 11, platform Win64/Orbis/Any.
    public class BigFileV11 : Big.IDependentArchive<ulong>
    {
        ...
        // Serialize: write a v11 .fat (FAT5 magic, version 11, flags, archive hash,
        // dependency count + deps, entry count + 20-byte entries). Entry format is
        // identical to v13 (same 20-byte layout); only the archive version differs.
        public void Serialize(Stream output)
        {
            ...
            output.WriteValueS32(11, Endian.Little);
            ...
            var entrySerializer = new Big.EntrySerializerV11();
        ...
        public void Deserialize(Stream input)
        {
            ...
            if (version != 11)
            {
                throw new FormatException(string.Format("unsupported version {0} (expected 11)", version));
            }
            ...
            {
                // v11 dependencies: 16 bytes (8+8) — same as v13.
            ...
            // Skip any trailing metadata (duplicates/localization) to end of stream.
            try
            {
        ...
        {
            return Big.CompressionSchemeV6.ToCompressionScheme(id);
        }

        public byte FromCompressionSCheme(Big.CompressionScheme compressionScheme)
        {
            return Big.CompressionSchemeV6.FromCompressionScheme(compressionScheme);
        }
    }
}